...
export class AuthService {
  ...
  private issueToken(
    ...
    // JWT role = granular Staff role (owner/manager/stylist/colorist) OR 'client'.
    // users.role ('owner'|'staff'|'client') is coarse; staff.role has the full granularity.
  ...
  async register(dto: RegisterDto): Promise<{ token: string; user: PublicUser }> {
    ...
    // Identifier already registered → conflict.
    ...
    // Merge-on-phone : relier un walk-in existant si présent.
  ...
  async createStaff(
    ...
    // StaffProfile pour stylists/colorists.
