 ...
 * Le scope tenant est résolu par le plugin (TenantContext, Prompt 3/6b). Enveloppe standard.
 */
@ApiTags('Clients')
@ApiBearerAuth()
@Controller('clients')
...
export class ClientsController {
  ...
  async list(
    ...
    const data = await this.clients.findAll(query.q);
  ...
  async detail(
    ...
    const data = await this.clients.findOne(id);
    return { data, message: 'OK' };
  }
  ...
  async create(
    ...
    const data = await this.clients.create(dto);
  ...
  async update(
    ...
    const data = await this.clients.update(id, dto);
