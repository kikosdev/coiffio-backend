import { Body, Controller, Delete, Get, Param, Patch, Post, Query, Res, UseGuards } from '@nestjs/common';
import { Response } from 'express';
import { ApiBearerAuth, ApiOperation, ApiResponse, ApiTags } from '@nestjs/swagger';
import { FinanceService, Period } from './finance.service';
import { CreatePaymentDto, CreateExpenseDto, UpdateExpenseDto, ReportsQueryDto, EarningsQueryDto } from './dto/finance.dto';
import { JwtGuard } from '../common/guards/jwt.guard';
import { RolesGuard } from '../common/guards/roles.guard';
import { Roles } from '../common/decorators/roles.decorator';
import { CurrentUser, AuthUser } from '../common/decorators/current-user.decorator';
import { FeatureGuard } from '../common/entitlements/guards/feature.guard';
import { RequiresFeature } from '../common/entitlements/decorators/requires-feature.decorator';
import { Destructive } from '../common/decorators/destructive.decorator';

/**
 * Finance / La Caisse (Sprint 5). Encaissement (owner·manager·stylist), vue split #9,
 * refund owner-only #8, dépenses + rapports + export CSV (owner·manager). Enveloppe + scope.
 */
@ApiTags('Finance')
@ApiBearerAuth()
@Controller()
@UseGuards(JwtGuard, RolesGuard)
export class FinanceController {
  constructor(private readonly finance: FinanceService) {}

  @ApiOperation({ summary: 'Record a payment for an appointment' })
  @ApiResponse({ status: 201, description: 'Payment recorded.' })
  @Post('payments')
  @Roles('owner', 'manager', 'stylist')
  async pay(@Body() dto: CreatePaymentDto) {
    const data = await this.finance.createPayment(dto);
    return { data, message: 'Payment recorded.' };
  }

  @ApiOperation({ summary: "Get the current staff member's own cash register (caisse) summary" })
  @ApiResponse({ status: 200, description: 'OK' })
  @Get('caisse/me')
  @Roles('owner', 'manager', 'stylist')
  async myCaisse(@CurrentUser() user: AuthUser) {
    const data = await this.finance.myCaisse(user);
    return { data, message: 'OK' };
  }

  @ApiOperation({ summary: "Get the current staff member's own earnings for a given period, with a by-service breakdown and chart bars" })
  @ApiResponse({ status: 200, description: 'OK' })
  @Get('earnings/me')
  @Roles('owner', 'manager', 'stylist')
  async myEarnings(@CurrentUser() user: AuthUser, @Query() query: EarningsQueryDto) {
    const data = await this.finance.myEarnings(user, query.period ?? 'week');
    return { data, message: 'OK' };
  }

  @ApiOperation({ summary: 'Get the salon-wide cash register (caisse) overview' })
  @ApiResponse({ status: 200, description: 'OK' })
  @Get('caisse/overview')
  @Roles('owner', 'manager')
  async overview() {
    const data = await this.finance.overview();
    return { data, message: 'OK' };
  }

  @ApiOperation({ summary: 'Refund a payment (owner only)' })
  @ApiResponse({ status: 201, description: 'Payment refunded.' })
  @Post('payments/:id/refund')
  @Roles('owner')
  @Destructive()
  async refund(@CurrentUser() user: AuthUser, @Param('id') id: string) {
    const data = await this.finance.refund(user, id);
    return { data, message: 'Payment refunded.' };
  }

  @ApiOperation({ summary: 'List expenses for the salon' })
  @ApiResponse({ status: 200, description: 'OK' })
  @Get('expenses')
  @Roles('owner', 'manager')
  async listExpenses() {
    const data = await this.finance.listExpenses();
    return { data, message: 'OK' };
  }

  @ApiOperation({ summary: 'Record a new expense' })
  @ApiResponse({ status: 201, description: 'Expense recorded.' })
  @Post('expenses')
  @Roles('owner', 'manager')
  async createExpense(@CurrentUser() user: AuthUser, @Body() dto: CreateExpenseDto) {
    const data = await this.finance.createExpense(user, dto);
    return { data, message: 'Expense recorded.' };
  }

  @ApiOperation({ summary: 'Update an existing expense' })
  @ApiResponse({ status: 200, description: 'Expense updated.' })
  @Patch('expenses/:id')
  @Roles('owner', 'manager')
  async updateExpense(@Param('id') id: string, @Body() dto: UpdateExpenseDto) {
    const data = await this.finance.updateExpense(id, dto);
    return { data, message: 'Expense updated.' };
  }

  @ApiOperation({ summary: 'Delete an expense' })
  @ApiResponse({ status: 200, description: 'Expense deleted.' })
  @Delete('expenses/:id')
  @Roles('owner', 'manager')
  @Destructive()
  async deleteExpense(@Param('id') id: string) {
    const data = await this.finance.deleteExpense(id);
    return { data, message: 'Expense deleted.' };
  }

  @ApiOperation({ summary: 'Get a financial report for a given period' })
  @ApiResponse({ status: 200, description: 'OK' })
  @Get('reports')
  @Roles('owner', 'manager')
  @UseGuards(FeatureGuard)
  @RequiresFeature('analytics')
  async report(@Query() query: ReportsQueryDto) {
    const data = await this.finance.report(query.period ?? 'day');
    return { data, message: 'OK' };
  }

  @ApiOperation({ summary: 'Export a financial report for a given period as CSV' })
  @ApiResponse({ status: 200, description: 'CSV file download.' })
  @Get('reports/export.csv')
  @Roles('owner', 'manager')
  @UseGuards(FeatureGuard)
  @RequiresFeature('analytics')
  async exportCsv(@Query() query: ReportsQueryDto, @Res() res: Response) {
    const period: Period = query.period ?? 'day';
    const csv = await this.finance.exportCsv(period);
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="report-${period}.csv"`);
    res.send(csv);
  }
}
